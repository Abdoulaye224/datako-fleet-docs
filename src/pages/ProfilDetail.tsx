...
export function ProfilDetail() {
  ...
  const { setProfilActif, progress, toggleProgress, resetProgress } = useProfil()
    ...
    <PageTransition>
      <div className="space-y-10">
        <div className="flex items-center gap-2 text-xs text-[var(--text-muted)]">
          <Link to="/profils" className="flex items-center gap-1 transition-colors hover:text-[var(--text-secondary)]">
            <ArrowLeft size={12} />
            Profils
          </Link>
          <span>›</span>
          <span className="text-[var(--text-secondary)]">{profil.nom}</span>
        </div>

        <section className="rounded-3xl border border-[var(--border)] bg-surface-2 p-6">
          <div className="flex flex-wrap items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-2xl font-bold text-[var(--text-primary)]">{profil.nom}</h1>
              <p className="mt-1 text-sm text-[var(--text-secondary)]">{profil.sousTitre}</p>
              <p className="mt-4 max-w-3xl text-sm leading-relaxed text-[var(--text-secondary)]">{profil.description}</p>
        ...
        <section className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h2 className="text-lg font-semibold text-[var(--text-primary)]">Parcours recommandé</h2>
            {progress.size > 0 && (
              <button
                type="button"
          ...
          <ProfilProgress done={progress.size} total={profil.parcoursRecommande.length} />
          <motion.div variants={VARIANTS.stagger} initial="initial" animate="animate" className="space-y-3">
            {profil.parcoursRecommande.map((article, index) => {
              const isDone = progress.has(index)
              return (
                <motion.div
                  ...
                  className={`flex items-center gap-3 rounded-2xl border p-4 transition-colors ${
                    isDone
                      ...
                      : 'border-[var(--border)] bg-surface-2 hover:border-blue-500/25 hover:bg-surface-3'
                  }`}
                >
                  <button
                    type="button"
                    onClick={() => toggleProgress(index)}
                    className="flex-shrink-0 text-[var(--text-muted)] transition-colors hover:text-emerald-400"
                    aria-label={isDone ? 'Marquer comme non lu' : 'Marquer comme lu'}
                  >
                    <motion.div
                      initial={false}
                      animate={{ scale: isDone ? [1.2, 1] : 1 }}
        ...
        <section className="space-y-4">
          <h2 className="text-lg font-semibold text-[var(--text-primary)]">Erreurs fréquentes</h2>
        ...
        <section className="space-y-4">
          <h2 className="text-lg font-semibold text-[var(--text-primary)]">Exports disponibles</h2>
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
