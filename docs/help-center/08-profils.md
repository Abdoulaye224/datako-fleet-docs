# 08 — Navigation par profil utilisateur

## Principe

La navigation par profil est une première classe dans l'application, pas un filtre secondaire.

À l'entrée sur le site, l'utilisateur choisit son profil. Ce choix :
- persiste dans `localStorage` (`hc-profil`)
- filtre la sidebar pour ne montrer que le contenu pertinent
- génère un parcours recommandé personnalisé
- peut être changé à tout moment depuis l'en-tête

Un utilisateur qui ne choisit pas de profil voit **toute** la documentation (mode "Tout afficher").

---

## UX de sélection du profil

### Point d'entrée — Page Accueil

Après la barre de recherche, bloc "Quel est votre profil ?" avec des cartes interactives :

```
┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐
│ 🚛       │ │ 🏪       │ │ 📊       │ │ 🔑       │ │ 🏢       │
│Transport │ │Marketeur │ │ DG /     │ │Propriét. │ │Comptable │
│          │ │Distrib.  │ │Direction │ │de flotte │ │/ Finance │
└──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘
┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐
│ 🚛🏪     │ │ 🔧       │ │ 📦       │ │ ⚡       │ │          │
│ Mixte    │ │Opérateur │ │Resp.     │ │Super     │ │ Tout     │
│Transp+Di │ │          │ │Logistique│ │Admin     │ │ afficher │
└──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘
```
...
Page complète avec :
- Grille des profils (cartes expandables au hover)
- Description courte de chaque profil
- "Ce que vous verrez" : 3 actions typiques
- Bouton "Choisir ce profil" → sauvegarde + redirect vers le parcours

### Header — changement de profil

Pill visible dans le header desktop et menu mobile :
```
[🚛 Transporteur ▾]  → dropdown pour changer de profil
```
...
```typescript
interface ProfilArticle {
  href: string          // route vers l'article
...
interface ProfilAction {
  titre: string
  guide: string         // href vers le guide
}

interface ProfilErreur {
  situation: string
  consequence: string
  solution: string
  articleLie?: string
...
interface ProfilIndicateur {
  nom: string
  pourquoi: string
  href: string
}

interface ProfilExport {
  nom: string
  quand: string
  comment: string
}
...
interface Profil {
  ...
  parcoursRecommande: ProfilArticle[]    // articles dans l'ordre de lecture
  actionsTupiques: ProfilAction[]
  erreursFréquentes: ProfilErreur[]
  indicateurs: ProfilIndicateur[]
  exports: ProfilExport[]
  rolesFleetManager: string[]            // rôles Fleet Manager correspondants
}
```
...
### 3. Profil mixte — Transport + Distribution
...
```typescript
{
  ...
  parcoursRecommande: [
    ...
    { href: '/portail-proprietaire/mes-vehicules', label: 'Voir mes véhicules et leurs rotations', priorite: 'critique' },
    { href: '/portail-proprietaire/mes-gains', label: 'Comprendre mes gains par rotation', priorite: 'critique' },
    { href: '/portail-proprietaire/mon-bilan', label: 'Lire mon bilan mensuel', priorite: 'importante' },
    { href: '/indicateurs/part-proprietaire', label: 'Comment est calculée ma part', priorite: 'importante' },
    { href: '/portail-proprietaire/bilan-pdf', label: 'Télécharger mon bilan PDF', priorite: 'utile' },
  ],

  actionsTupiques: [
    { titre: 'Vérifier les rotations effectuées ce mois par mes camions', guide: '/portail-proprietaire/mes-vehicules' },
    { titre: 'Calculer ma part de gains sur le mois', guide: '/portail-proprietaire/mes-gains' },
    { titre: 'Télécharger mon bilan PDF pour validation', guide: '/portail-proprietaire/bilan-pdf' },
  ],
...
Quand un profil est sélectionné, la sidebar affiche en tête une section "Mon parcours" :

```
┌─────────────────────────────┐
│  🚛 Transporteur  [Changer] │
├─────────────────────────────┤
│  Mon parcours               │
│  ✅ Cycle d'une rotation    │
│  ✅ Créer une rotation      │
│  ☐  Valider une livraison   │  ← prochain à lire
│  ☐  Lire le Dashboard       │
│  ...                        │
├─────────────────────────────┤
│  Transport                  │
...
Progression : stockée dans `localStorage` (`hc-profil-progress-{id}`), cases à cocher manuelles.
...
Modules concernés : [badges modules]
Rôle Fleet Manager correspondant : [badge rôle]

Mon parcours recommandé
────────────────────────
① [critique] Comprendre le cycle d'une rotation  → Lire
② [critique] Créer une rotation                   → Lire
③ [importante] ...

Actions typiques
─────────────────
· Enregistrer une livraison → [Guide]
· ...
...
[Choisir ce profil]    [Voir toute la documentation]
```
